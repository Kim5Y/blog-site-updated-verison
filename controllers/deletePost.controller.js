...
export default async (req, res) => {
  try {
    ...
    if (!postId)
      return new ApiError(res, { statuscode: 400, message: "invalid post ID" });
    const query = `SELECT * FROM posts WHERE id = $1`;
    const result = await pool.query(query, [postId]);
    if(result.user_id !== req.user.id) return new ApiError(res, {message: "invalid", statuscode: 403})
    ...
    if (deltedPost) {
      (await io).to(`category-${category}`).emit("post:deleted", POST);
    }
